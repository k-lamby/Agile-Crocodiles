...
// ================================= functions for addbooks.ejs ==============================
    ...
    app.post("/addbooks/search", (req, res) =>{
        ...
        request(url, {json: true}, (err, response, body)=> {
            ...
            if(err || typeof body.items == 'undefined') {
                flag = "n"
                req.session.message = {
                    type: 'danger',
                    intro: 'Fail!',
                    message: 'We have not been able to locate this book. Try again!'
                }
                ...
                for(let i = 0; i < body.items.length; i++){
                    ...
                    else
                    {
                        ...
                        try{
                            info.description = body.items[i].volumeInfo.description.substring(0, 200).replace(/[^\w .,;:'"?!]/g, "");
    ...
    app.post("/addbooks/add", (req, res) => {
        ...
        setTimeout(()=> console.log("Waiting for the book to be added"), 2000);
        let query2 = ['INSERT INTO title (name, authorID, ISBN, adultContent, genreID, description) VALUES ("' + req.body.title + '",'
                        + '(SELECT ID FROM author WHERE author = "'+ req.body.author+ '")' + ',"'
                        + req.body.ISBN + '",' + adultContent + ', (SELECT ID FROM genre WHERE name = "' + req.body.genre + '"), "' + req.body.description + '")', 
...
// ================================= functions for match.ejs ================================== 
    app.get("/match",(req, res) => {
        ...
        if (userSetting.length == 0){ 
            query.push("SELECT title.name, author.author, cover.link FROM author RIGHT JOIN title ON author.ID = title.authorID LEFT JOIN cover ON title.ID = cover.titleID WHERE genreID = (SELECT ID FROM genre WHERE name = 'nogenre')"); 
        }
    ...
    app.post("/match/submit", (req, res)=> {
        ...
        connection.query(query, (err) => {
            if (err) console.log("A book is added to the wish list.");;
...
// ================================= functions for wishlist.ejs ================================= 
    ...
    app.post("/wishlist/oneliner", (req, res) => {
        bookInfo.splice(bookInfo.findIndex((element) => element.title == req.body.title), 1); // remove book from the array
        let oneline = req.body.oneliner.replace(/[^\w .,;:'"?!]/g, ""); // remove non-alphanumeric characters, except a number of special characters and space.
        let query = ['INSERT INTO oneliner VALUES ((SELECT ID FROM title WHERE name = "' + req.body.modalTitle + '"), 9 ,"' + oneline+ '")'];
        connection.query(query.join(';'), (err, results) => {
...
//formatData function creates a JS object which stores certain information about a book and push it to the array for the wish list.
    function formatData(queryResults)
    {
        let obj = {
            ...
            genre : queryResults[3][0].name,
            description : queryResults[4][0].description.replace(/[^\w .,;:'"?!]/g, "")
        };
        bookInfo.push(obj);
    }
            ...
            for(let i = 0; i < results.length; i++)
                    {
                        let innerQuery = ['SELECT name FROM title WHERE ID = ' + results[i].titleID,
                                        ...
                                        'SELECT name FROM genre WHERE ID = (SELECT genreID FROM title WHERE ID = ' + results[i].titleID + ')',
                                        'SELECT description FROM title WHERE ID = ' + results[i].titleID];
